...
int inflateInit2_(z_streamp strm, int windowBits, const char *version, int stream_size) {
    ...
    if (windowBits != 15 + 16) {
        strm->msg = (char*)"invalid windowBits parameter -- 31 only supported value";
        return Z_STREAM_ERROR;
    }
    windowBits -= 16;
    assert(8 <= windowBits && windowBits <= 15);

    void *mem = strm->zalloc(strm->opaque, 1, static_cast<uInt>(sizeof(internal_state)));
    if (!mem) {
        strm->msg = (char*)"failed to allocate memory for internal state";
        return Z_MEM_ERROR;
    }
...
{
    (void)flush;
    strm->msg = const_cast<char*>("Bahhhhh");
    return Z_DATA_ERROR;
}
